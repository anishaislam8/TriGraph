    ...
    while(!myfile_test.eof()){
        ...
        try{
            ...
            for (auto subgraph: three_node_subgraphs_test){
                rank = predict_edges(subgraph, object_dict_test, frequency_2_grams_map, frequency_3_grams_map, sum_frequency_2_grams, sum_frequency_3_grams, unique_tokens_train_map, G_directed_test,1);
